...
if(!empty($_POST['settings'])){
	if(configGet('fblogin') != $_POST['fblogin']) {
		$fblogin = Input::get('fblogin');
		$fields=array('fblogin'=>$fblogin);
		$db->update('settings',1,$fields);
	}
	if(configGet('fbid') != $_POST['fbid']) {
		$fbid = Input::get('fbid');
		$fields=array('fbid'=>$fbid);
		$db->update('settings',1,$fields);
	}
	if(configGet('fbsecret') != $_POST['fbsecret']) {
		$fbsecret = Input::get('fbsecret');
		$fields=array('fbsecret'=>$fbsecret);
		$db->update('settings',1,$fields);
	}
	if(configGet('fbcallback') != $_POST['fbcallback']) {
	...
	<div class="col-xs-12"> <!-- Site Settings Column -->
		...
		<div class="form-group">
			...
			<select id="fblogin" class="form-control" name="fblogin">
				<option value="1" <?php if(configGet('fblogin')==1) echo 'selected="selected"'; ?> >Enabled</option>
				<option value="0" <?php if(configGet('fblogin')==0) echo 'selected="selected"'; ?> >Disabled</option>
			</select>
		</div>
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" name="fbid" id="fbid" value="<?=configGet('fbid')?>">
		</div>

		<!-- fbsecret -->
		<div class="form-group">
			<label for="fbsecret">Secret</label>
			<input type="text" class="form-control" name="fbsecret" id="fbsecret" value="<?=configGet('fbsecret')?>">
		</div>

		<!-- fbcallback -->
		<div class="form-group">
			<label for="fbcallback">Callback</label>
			<input type="text" class="form-control" name="fbcallback" id="fbcallback" value="<?=configGet('fbcallback')?>">
		</div>
