...
if (Input::get('forgotten_password')) {
	...
	if($validation->passed()){
		if($fuser->exists()){
			...
			*/
			$url='<a href="'.configGet('site_url').'/users/password_reset.php?email='.rawurlencode($email).'&vericode='.$fuser->data()->vericode.'&reset=1">Reset Password</a>';
			$options = array(
			  'fname' => $fuser->data()->fname,
			  'url' => $url,
			  'sitename' => configGet('site_name'),
			);
			$subject = 'Password Reset';
			$body =  email_body(configGet('forgot_password_template'),$options);
