...
if(!empty($_POST['settings'])){
	if(configGet('glogin') != $_POST['glogin']) {
		$glogin = Input::get('glogin');
		$fields=array('glogin'=>$glogin);
		$db->update('settings',1,$fields);
	}
	if(configGet('gid') != $_POST['gid']) {
		$gid = Input::get('gid');
		$fields=array('gid'=>$gid);
		$db->update('settings',1,$fields);
	}
	if(configGet('gsecret') != $_POST['gsecret']) {
		$gsecret = Input::get('gsecret');
		$fields=array('gsecret'=>$gsecret);
		$db->update('settings',1,$fields);
	}
	if(configGet('gcallback') != $_POST['gcallback']) {
	...
	<div class="col-xs-12"> <!-- Site Settings Column -->
		...
		<div class="form-group">
			...
			<select id="glogin" class="form-control" name="glogin">
				<option value="1" <?php if(configGet('glogin')==1) echo 'selected="selected"'; ?> >Enabled</option>
				<option value="0" <?php if(configGet('glogin')==0) echo 'selected="selected"'; ?> >Disabled</option>
			</select>
		</div>
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" name="gid" id="gid" value="<?=configGet('gid')?>">
		</div>

		<!-- gsecret -->
		<div class="form-group">
			<label for="gsecret">Secret</label>
			<input type="text" class="form-control" name="gsecret" id="gsecret" value="<?=configGet('gsecret')?>">
		</div>

		<!-- gredirect -->
		<div class="form-group">
			<label for="gcallback">Callback</label>
			<input type="text" class="form-control" name="gcallback" id="gcallback" value="<?=configGet('gcallback')?>">
		</div>
