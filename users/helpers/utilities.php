...
function backupUsTables($tableNames,$backupPath){
	global $dbHost, $dbUsername, $dbPassword, $dbDatabase;
	/*
	Backs up one table at a time so each resulting file can be used independently
	*/
	if($tableNames!=null && $backupPath!=null){
		foreach($tableNames as $us_table){
			$userspiceDump = Shuttle_Dumper::create(array('host' => $dbHost,'username' => $dbUsername,'password' => $dbPassword,'db_name' => $dbDatabase,'include_tables' => (array)$us_table));
			$userspiceDumpFilename=$backupPath.$us_table.'.sql';
			$userspiceDump->dump($userspiceDumpFilename);
			$successes[]='UserSpice table "'.$us_table.'" has been backed up to '.$userspiceDumpFilename.'.';
		}	
	}else{
		$errors[]='backupUsTables function was not called correctly.';
		return false;
	}
	return true;
}
