...
if(!empty($_POST['settings'])){
	if(configGet('site_name') != $_POST['agreement']) {
		$agreement = Input::get('agreement');
		$fields=array('agreement'=>$agreement);
		$db->update('settings',1,$fields);
	}

	if(configGet('email_act') != $_POST['email_act']) {
		...
		<div class="form-group">
		  <label for="radios">Require Email Verification</label>
			<div class="">
				...
				<input type="radio" name="email_act" id="email_act_1" value="1" <?php echo (configGet('email_act')==1)?'checked':''; ?>>Yes</label>
				<label class="radio-inline" for="email_act_0">
				<input type="radio" name="email_act" id="email_act_0" value="0" <?php echo (configGet('email_act')==0)?'checked':''; ?>>No</label>
			</div>
		</div>

		<div class="form-group">
			<label>Terms and Conditions</label>
			<textarea class="form-control" rows="10" id="agreement" name="agreement" ><?=configGet('agreement');?></textarea></p>
		</div>
