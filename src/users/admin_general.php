...
if(!empty($_POST['settings'])){
	...
	if($site_settings->site_name != $_POST['site_name']) {
		$site_name = Input::get('site_name');
		$fields=array('site_name'=>$site_name);
		$db->update('settings',1,$fields);
	}
	if($site_settings->site_url != $_POST['site_url']) {
	...
	if($site_settings->language != $_POST['language']) {
	...
	if($site_settings->site_offline != $_POST['site_offline']) {
		$site_offline = Input::get('site_offline');
		$fields=array('site_offline'=>$site_offline);
		$db->update('settings',1,$fields);
	}
	if($site_settings->debug_mode != $_POST['debug_mode']) {
...
<div class="row"> <!-- rows for Main Settings -->
	<div class="col-xs-12"> <!-- Site Settings Column -->
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" name="install_location" id="install_location" value="<?=$site_settings->install_location?>">
		</div>
		...
		<div class="form-group">
			...
			<select id="recaptcha" class="form-control" name="recaptcha">
				<option value="1" <?php if($site_settings->recaptcha==1) echo 'selected="selected"'; ?> >Enabled</option>
				<option value="0" <?php if($site_settings->recaptcha==0) echo 'selected="selected"'; ?> >Disabled</option>
			</select>
		</div>
		...
		<div class="form-group">
			...
			<select id="site_offline" class="form-control" name="site_offline">
				<option value="1" <?php if($site_settings->site_offline==1) echo 'selected="selected"'; ?> >Yes</option>
		...
		<div class="form-group">
			...
			<select id="track_guest" class="form-control" name="track_guest">
				<option value="1" <?php if($site_settings->track_guest==1) echo 'selected="selected"'; ?> >Yes</option>
				<option value="0" <?php if($site_settings->track_guest==0) echo 'selected="selected"'; ?> >No</option>
