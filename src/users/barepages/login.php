	...
	if ($reCaptchaValid || configGet('recaptcha') == 0) { //if recaptcha valid or recaptcha disabled
		...
		if ($validation->passed()) {
			...
			if ($login) {
				...
				*/
				if ($_SESSION['securePageRequest'] && configGet('redirect_referrer_login')) {
					//bold('HERE');
...
<div class="row">
	...
	<?php
	if (configGet('allow_remember_me') == 1) {
	?>
	<div class="form-group">
	...
	<div class="text-center">
		...
		<?php	if(configGet('glogin')){?><a href="<?=$gAuthUrl?>" type="button"><img src="<?=US_URL_ROOT.'users/images/google.png'?>" height="35px"></a><?php } ?>
		<?php if(configGet('fblogin')){?><a href="<?=$fbAuthUrl?>" type="button"><img src="<?=US_URL_ROOT.'users/images/facebook.png'?>" height="35px"></a><?php } ?>
