...
if(!empty($_POST['settings'])){
	...
	if($site_settings->recaptcha_private != $_POST['recaptcha_private']) {
		$recaptcha_private = Input::get('recaptcha_private');
		$fields=array('recaptcha_private'=>$recaptcha_private);
		$db->update('settings',1,$fields);
	}
	if($site_settings->recaptcha_public != $_POST['recaptcha_public']) {
	...
	if($site_settings->allow_remember_me != $_POST['allow_remember_me']) {
...
<div class="row"> <!-- rows for Main Settings -->
	<div class="col-xs-12"> <!-- Site Settings Column -->
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" name="recaptcha_private" id="recaptcha_private" value="<?=$site_settings->recaptcha_private?>">
		</div>
