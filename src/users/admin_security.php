...
if(!empty($_POST['settings'])){
	...
	if($$cfg->get('force_ssl') != $_POST['force_ssl']) {
	...
	<div class="col-xs-12"> <!-- Site Settings Column -->
		...
		<div class="form-group">
			...
			<select id="force_ssl" class="form-control" name="force_ssl">
				<option value="1" <?php if($$cfg->get('force_ssl')==1) echo 'selected="selected"'; ?> >Yes</option>
				<option value="0" <?php if($$cfg->get('force_ssl')==0) echo 'selected="selected"'; ?> >No</option>
			</select>
		</div>
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" name="recaptcha_private" id="recaptcha_private" value="<?=$$cfg->get('recaptcha_private')?>">
		</div>

		<!-- recaptcha_public -->
		<div class="form-group">
			<label for="recaptcha_public">Recaptcha Public Key</label>
			<input type="text" class="form-control" name="recaptcha_public" id="recaptcha_public" value="<?=$$cfg->get('recaptcha_public')?>">
		</div>

		<!-- session_timeout -->
		<div class="form-group">
			<label for="session_timeout">Session Timeout (in seconds: 3600 = 1 hour)</label>
			<input type="text" class="form-control" name="session_timeout" id="session_timeout" value="<?=$$cfg->get('session_timeout')?>">
		</div>
		...
		<div class="form-group">
			...
			<select id="allow_remember_me" class="form-control" name="allow_remember_me">
				<option value="1" <?php if($$cfg->get('allow_remember_me')==1) echo 'selected="selected"'; ?> >Yes</option>
				<option value="0" <?php if($$cfg->get('allow_remember_me')==0) echo 'selected="selected"'; ?> >No</option>
			</select>
		</div>
